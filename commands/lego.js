...
exports.run = async (bot, message, args) => {
  ...
  message.reply("_*building...*_").catch(console.error);

  await sharp(`images/doges/${item.id}.png`)
  .composite([
    { input: 'images/effects/lego.png', gravity: 'southwest' }
  ])
  .toBuffer((err, data, info) => {
    console.log(data);
    message.reply({ files: [{ attachment: data, name: `${item.id}-lego.png`  }] }).catch(console.error);
  });
}
