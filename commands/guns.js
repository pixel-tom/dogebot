...
exports.run = async (bot, message, args) => {
  ...
  message.reply("_*click click*_").catch(console.error);

  await sharp(`images/doges/${item.id}.png`)
  .composite([
    { input: 'images/effects/guns.png', gravity: 'southwest' }
  ])
  .toBuffer((err, data, info) => {
    console.log(data);
    message.reply({ files: [{ attachment: data, name: `${item.id}-gun.png`  }] }).catch(console.error);
  });
}
